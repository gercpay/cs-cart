...
use Tygh\Http;
use Tygh\Registry;
use Tygh\Payments\Processors\Gercpay as Gercpay;
...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    switch ($mode) {
        case 'approve':
            fn_set_notification('N', __('gercpay.payment_approve'), __('gercpay.payment_approve_text'));
            break;
        case 'decline':
            fn_set_notification('E', __('gercpay.payment_decline'), __('gercpay.payment_decline_text'));
            break;
        case 'cancel':
            fn_set_notification('W', __('gercpay.payment_cancel'), __('gercpay.payment_cancel_text'));
            break;
        case 'callback':
    ...
    if ($mode === 'callback') {
        ...
        $gercpay = new Gercpay($processor_params['merchantAccount'], $processor_params['merchantSecretKey']);

        if (empty($data['cardPan'])) {
            $data['cardPan'] = '';
        }

        $valid_signature = $gercpay->createSignature('CHECK_RESPONSE', $data);
        if ($data['merchantSignature'] !== $valid_signature) {
            return array(CONTROLLER_STATUS_NO_PAGE);
        }

        $pp_response = $gercpay->getFieldsNameForOrder($data, $processor_params['order_status']);
...
$params = array(
    ...
    'currency_iso' => Gercpay::CURRENCY_UAH,
    'description'  => __('gercpay.payment.card_payment') . ' ' . htmlspecialchars($_SERVER["HTTP_HOST"]) . ', ' .
        $client_first_name . ' ' . $client_last_name . ', ' . $client_phone,
    'add_params'   => [],
    'approve_url'  => fn_url('payment_notification.approve&payment=gercpay'),
    'decline_url'  => fn_url('payment_notification.decline&payment=gercpay'),
    'cancel_url'   => fn_url('payment_notification.cancel&payment=gercpay'),
    'callback_url' => fn_url('payment_notification.callback&payment=gercpay'),
    // Statistics.
...
$form = $gercpay->buildForm($params);
echo __('text_cc_processor_connection', ['[processor]' => __('gercpay')]);
echo $form;
echo <<<EOT
        <noscript><p>
EOT;
